...
type GridProps = BaseProps & {
  ...
  /* TODO buildResponsiveRules should be able to handle this case (array of strings) */
...
export const Grid = styled.div<GridProps>`
  ...
  ${({ align }) =>
    align &&
    buildResponsiveRules<JustifyAlignContentValues>('align-content', align)}
  ${({ areas }) =>
    areas &&
    buildResponsiveRules<string>(
      'grid-template-areas',
      convertAreaValues(areas),
    )}
`;
...
export const GridItem = styled.div<GridItemProps>`
  ...
  ${({ top }) =>
    top && buildResponsiveRules<string | number>('grid-row-start', top)}
  ...
  ${({ align }) =>
    align &&
    buildResponsiveRules<JustifyAlignContentValues>('align-self', align)}
  ${({ middle }) =>
    middle &&
    css`
      display: inline-flex;
...
const convertAreaValues = (
  ...
  if (typeof areasValue[0] === 'string') {
    // if the first el is a string then we know user is using a single default value rather than the tuple
